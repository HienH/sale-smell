...
export class AssemblyAIService {
    ...
    async uploadAndTranscribe(
        ...
        try {
            ...
            // Create transcription request with basic features first
            ...
            throw this.handleError(error, 'Failed to upload and transcribe audio file');
        }
    }
    ...
    private mapTranscriptionToResult(transcription: any): TranscriptionResult {
        ...
        } else if (transcription.speaker_labels && Array.isArray(transcription.speaker_labels)) {
        ...
        } else if (transcription.speaker_labels && transcription.speaker_labels.segments) {
            result.speakers = transcription.speaker_labels.segments.map((segment: any) => ({
                speaker: segment.speaker,
                text: segment.text,
                start: segment.start,
                end: segment.end,
        ...
        if (transcription.sentiment_analysis_results && Array.isArray(transcription.sentiment_analysis_results)) {
            result.sentiment = {
                ...
                segments: transcription.sentiment_analysis_results.map((segment: any) => ({
                    text: segment.text,
            ...
            const sentimentCounts = result.sentiment.segments.reduce((acc: any, segment: any) => {
                if (segment.sentiment && typeof segment.sentiment === 'string') {
                    acc[segment.sentiment.toLowerCase()] = (acc[segment.sentiment.toLowerCase()] || 0) + 1;
                }
                return acc;
            }, {});
            ...
            const total = result.sentiment.scores.positive + result.sentiment.scores.negative + result.sentiment.scores.neutral;
            if (total > 0) {
                if (result.sentiment.scores.positive > result.sentiment.scores.negative) {
                    result.sentiment.overall = 'POSITIVE';
                } else if (result.sentiment.scores.negative > result.sentiment.scores.positive) {
        ...
        if (transcription.pii_redaction_results && Array.isArray(transcription.pii_redaction_results)) {
            result.piiRedactions = transcription.pii_redaction_results.map((redaction: any) => ({
                text: redaction.text,
                type: redaction.type,
        ...
        // Map auto highlights
        if (transcription.auto_highlights_result && Array.isArray(transcription.auto_highlights_result.results)) {
            result.autoHighlights = transcription.auto_highlights_result.results.map((highlight: any) => ({
                text: highlight.text,
    ...
    private handleError(error: any, context: string): Error {
        ...
        if (error?.response?.status) {
            switch (error.response.status) {
                case 401:
                    message = 'Invalid API key. Please check your AssemblyAI credentials.';
                    break;
                case 403:
